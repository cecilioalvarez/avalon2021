...
public class Estudio005_PasoPorReferencia2 {
	...
	static void imprimir(int[] nuevalista) {
		for (int i=0;i<nuevalista.length;i++) {
			System.out.println(nuevalista[i]);
		}
	}
	
	static void multiplicar(int[] pasoelementosoriginales) {
		for (int i=0;i<pasoelementosoriginales.length;i++) {
			pasoelementosoriginales[i]=pasoelementosoriginales[i]*5;
		}
	}
}
